...
//默认内置方法。
    window.layer = {
        ...
        alert: function(msg, icon, fn, yes,isCloseSucess){
...
//容器
    Class.pt.space = function(html){
        var that = this, html = html || '', times = that.index, config = that.config, dialog = config.dialog,
            ico = dialog.type === -1 ? '' : '',
            frame = [
                '<div class="xubox_dialog">' + dialog.msg + '</div>',
        ...
        config.closeBtn[1] && (closebtn += '<a class="xubox_close xulayer_png32 xubox_close' + config.closeBtn[0] +'" href="javascript:;" style="'+ (config.type === 4 ? 'position:absolute; right:-3px; _right:7px; top:-4px;' : '') +'"><span class="closeTip">关闭<em></em></span></a>');
...
//创建骨架
    Class.pt.creat = function(){
        ...
        //隐藏标题
        if(config.hideTitle){
            layerE.find('.xubox_title').hide();
            layerE.find('.xubox_dialog ').css({
                paddingTop: 0
            });
        };
...
//自适应宽高
    Class.pt.autoArea = function(times){
        ...
        (config.area[1] === 'auto') && layerMian.css({height: 'auto'});
...
//主入口
    ...
    if(/*"function" === typeof define*/ false){
        define(function(){
            ready.run();
            return layer;
        });
